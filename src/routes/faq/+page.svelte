...
<div class="min-h-screen bg-gradient-to-br from-[#004D56] via-[#00444B] to-[#003B41] p-4 sm:p-6 lg:p-8">
  <div class="max-w-4xl mx-auto">
    ...
    <div class="text-center mb-8">
      <h1 class="text-3xl font-bold text-white mb-3">{t(currentLanguage, 'faq.title')}</h1>
    ...
    {#if filteredFAQs.length === 0}
      <div class="text-center py-12">
        ...
        <h3 class="text-xl font-semibold text-white mb-2">{t(currentLanguage, 'faq.noResults.title')}</h3>
        ...
        <button 
          on:click={() => searchTerm = ''}
          class="px-4 py-2 bg-white/10 hover:bg-white/20 text-white transition-colors"
        >
    ...
    <div class="mt-12 text-center">
      <div class="bg-white/10 backdrop-blur-xl border border-white/30 p-8">
        <h3 class="text-2xl font-bold text-white mb-4">{t(currentLanguage, 'faq.stillQuestions.title')}</h3>
        <p class="text-white/70 mb-6">{t(currentLanguage, 'faq.stillQuestions.message')}</p>
        <a 
          href="/contact" 
          class="inline-flex items-center gap-2 bg-[#0C6E78] hover:bg-[#0a5a61] text-white px-6 py-3 font-medium transition-colors"
        >
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
